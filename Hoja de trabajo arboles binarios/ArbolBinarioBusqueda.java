public class ArbolBinarioBusqueda<K, T> implements IArbolBinario<K, T> {
	...
	public T Find(K key, IVisitar<K, T> visitNode, NodoArbol<K, T> actual) {
		// TODO Auto-generated method stub
		T respuesta = null;
		if (actual != null) {

			String keymandar = (String)key;

			respuesta = Find(key, visitNode, actual.hijoIzquierdo);
			if(respuesta != null){
			...
			respuesta = visitNode.VisitarNodoRegresa(actual, keymandar);
			if(respuesta != null){ 
			...
			respuesta = Find(key, visitNode, actual.hijoDerecho);
			if(respuesta != null){
